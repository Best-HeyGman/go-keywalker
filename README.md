...
Sometimes, when people want a "random" string of characters, they do what is called a keyboard walk. Don't know what to call you new movie? asdf. Very random. Passwords should be random, right? Oh no...

# The Keyboard Map file format (.nsk)
It's a `\n` separated utf8 text file  
The file format I created to map out a keyboard is as follows:  
- Every line of the text file lists one key and all of it's adjacent keys.
- The first character of a line is the key we are mapping out.
- Every character afterwards is a neighbouring key on the keyboard.

I have choosen this format to make mapping a new keyboard as easy as possible. I wanted a format where you don't have to look up from the keyboard while mapping it, because the task is boring enough for you to get easily distracted, but complex enough that you will make mistakes if you get distracted.
...
# Usage
## keywalk-gen
Generate all keyboard walks on a given keybaord.  
`keywalk-gen -k <path/to/your/keymap.nsk> -m <min_length> -M <max_length>`  
This will output every possible keywalk on the given keyboard and given lengths to stdout.
...
# libkeywalk
I made a library that is shared by these two programs. It only can do what I needed for the current functionality, but if you are interested and want to use it for example to check password quality in your program or something, just let me know and I make a real go package out of it (however that works) and document it thoroughly.
...
## My ideas
I also have a couple of ideas myself, but I don't really need them presonally, so I didn't add them yet, but if someone else needs them, I'll definitely add them:  
- Also map alternative key assignments like capital letters and the likes
...
- Multithreading
	...
	+ I've already split up the functions in libkeywalk in a way so that I can easily add this feature.
	+ But the performance without multithreading is already really good, so I didn't bother yet